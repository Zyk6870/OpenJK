...
void target_level_change_use(gentity_t *self, gentity_t *other, gentity_t *activator)
{
	...
	// trap->SendConsoleCommand(EXEC_NOW, va("map %s", self->message)); // zyk: this line was replaced by the code below. Credits to praviilon (Github issue #8) for finding this crash fix
