...
void ForceTeamForceReplenish( gentity_t *self )
{
	...
	while (i < level.num_entities)
	{
		...
		{
			max_blasterpack_ammo = zyk_max_blaster_pack_ammo.value + zyk_max_blaster_pack_ammo.value/ 8.0 * ent->client->pers.skill_levels[55];
			max_powercell_ammo = zyk_max_power_cell_ammo.value + zyk_max_power_cell_ammo.value / 8.0 * ent->client->pers.skill_levels[55];
		}
		else
		{
