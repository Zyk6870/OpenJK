	...
	if (client->pers.send_event_timer > level.time && client->pers.send_event_interval < level.time)
	{
		...
		{ // zyk: tells client-side mod to render the Force Shield effect
			if (client->sess.amrpgmode == 2 && (client->pers.active_unique_skill >= 4 && client->pers.active_unique_skill <= 9))
				G_AddEvent(ent, EV_USE_ITEM13, 2000);
			else
				G_AddEvent(ent, EV_USE_ITEM13, 3000);
		...
		(pmove.cmd.generic_cmd != ent->client->lastGenCmd || ent->client->lastGenCmdTime < level.time))
	{
		...
		{ //these are the only two where you wouldn't care about a delay between
			if (ent->client->sess.amrpgmode == 2)
			{
				...
				{ 
					if (ent->client->pers.rpg_upgrades & (1 << UPGRADE_ENERGY_MODULATOR) && ent->client->ps.weapon != WP_MELEE)
